...
def load_client_csvs() -> dict[str, pd.DataFrame]:
    ...
    for f in files:
        ...
        df["label"] = label
        # Extract algo name from label (rr, lc, ucb)
        algo = label.rsplit("_run", 1)[0]
        df["algorithm"] = algo
        frames[label] = df
    return frames
...
def algo_label(code: str) -> str:
    return {"rr": "Round Robin", "lc": "Least Connections", "ucb": "UCB"}.get(code, code)
